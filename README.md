# TP1
This project aims to simulate a bouncing balls, in free fall, inside a delimited box (1x1). It is desenvolved in C++ and compiled with G++ GNU Compiler version 5.4.0. As request, it has the `iostream` and `stdio` libraries.

## 1st Part
For this part the aim is to simulate one singular ball bouncing. And the files used are: `ball.h` `ball.cpp` `test-ball.cpp`
...
```
## 2st Part
For this part the attempt is to simulate two balls bouncing, being both connected by a spring, witch has a force to contract, bringing the balls closer together or further apart to singular ball bouncing. And the files used are: `springmass.h` `springmass.cpp` `test-springmass.cpp`
